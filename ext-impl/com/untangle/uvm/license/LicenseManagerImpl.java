...
/**xxxx turn off all of the logging when this is done */
...
{    
    ...
    /**
     * Reload all of the licenses from the filesystem.
     */
    ...
    public synchronized LicenseStatus getLicenseStatus( String identifier )
    {
        License license = this.licenseMap.get( identifier );
        
        /* Found a valid license */
        if ( license != null ) return makeLicenseStatus( identifier, license );
        ...
        return makeLicenseStatus( identifier, license );
    }
    ...
    private synchronized void readLicenses()
    {
        ...
        for ( File licenseFile : licenseFiles ) {
            try {
                ...
                String identifier = props.getProperty( PROPERTY_PRODUCT_IDENTIFIER );
                if ( identifier == null ) throw new Exception( "missing product id" );
                String mackage = props.getProperty( PROPERTY_MACKAGE );
                if ( mackage == null ) throw new Exception( "missing mackage id" );
                String type  = props.getProperty( PROPERTY_LICENSE_TYPE );
                if ( type == null ) throw new Exception( "missing license type" );
                long start  = Long.parseLong( props.getProperty( PROPERTY_START ));
                if ( start == 0 ) throw new Exception( "missing start date" );
                long end  = Long.parseLong( props.getProperty( PROPERTY_END ));
                if ( end == 0 ) throw new Exception( "missing end date" );
                ...
                if ( !verifyLicense( license, false )) {
                    // NO_DEBUG_IN_LOGGING logger.debug( "The license: " + license + " is not valid." );
                    continue;
                }
    ...
    /**
     * Update the extra name.
     */
    private synchronized void updateExtraName()
    {
        /**
         * This matches a mackage to the license that is going to
         * last, if they have both a trial and full license, it will
         * automatically use the full license.
         */
        Map<String,License> mackageState = new HashMap<String,License>();

        /* Create new map of all of the mackages to the latest license */
        for ( License license : this.licenseSettings.getLicenses()) {
            String mackage = license.getMackage();
            License l = mackageState.get( mackage );
            
            /* If there is no license, or the current license
             * expires later, use the new license */
            if (( l == null ) || ( l.getEnd() < license.getEnd())) {
                mackageState.put( mackage, license );
            }
        }

        /* Use now to calculate the number of days until this license expires */
        long now = System.currentTimeMillis();
        for ( Map.Entry<String,License> entry : mackageState.entrySet()) {
            License l = entry.getValue();
            String mackage = entry.getKey();
            long expiration = l.getEnd();

            String extraName = "";

            // NO_DEBUG_IN_LOGGING logger.debug( "Checking " + mackage + " against: " + l );
            ...
            /* Set the extra name, and enable/disable state */
            RemoteToolboxManager toolbox = LocalUvmContextFactory.context().toolboxManager();
            MackageDesc desc = toolbox.mackageDesc( mackage );

            if (( desc != null ) && ( desc.getType() == MackageDesc.Type.NODE )) {
                String currentExtraName = desc.getExtraName();
                if ( currentExtraName == null ) currentExtraName = "";

                if ( !extraName.equals( currentExtraName )) toolbox.extraName( mackage, extraName );
            } else {
        ...
        for ( Product product : products ) {
            try {
                ...
                if ( license == null ) {
                     ...
                     * product for a little longer there is no need to
    ...
    private boolean verifyLicense( License license, boolean verifyExpiration )
    {
        ...
        if (( license.getEnd() < now ) && verifyExpiration ) {
            // NO_DEBUG_IN_LM logger.debug( "The license: " + license + " has already expired." );
            return false;
        }
        ...
        if ( licenseType == null ) {
            // NO_DEBUG_IN_LM logger.debug( "unknown license type: '" + license.getType() + "'" );
            return false;
        }
        ...
        case SELF_SIGNED_VERSION:
            try {
                ...
                if ( !expected.equals( license.getKey())) {
                    // NO_DEBUG_IN_LM logger.debug( "Invalid license key for " + license );
    ...
    private String buildLicenseFile( License license )
    {
        ...
        /* just a little string to make it slightly harder to guess the algorithm */
        int version = license.getKeyVersion();
        switch ( version ) {
        case SELF_SIGNED_VERSION:
    ...
    private LicenseStatus makeLicenseStatus( String identifier, License license )
    {
        if ( license == null ) {
            return new LicenseStatus( false, identifier, "invalid", new Date( 0 ), EXPIRED, false );
        }
    ...
    private class LicenseUpdateTask implements Runnable
    {
        public void run()
        {
            // NO_DEBUG_IN_LOGGING logger.debug( "testing licenses and updating products." );
    ...
    {
        /* The name of the product to expire */
        private final Product product;

        private ExpirationTask( Product product )
        {
        ...
        public void run()
        {
            /* not sure if this needs to be synchronized since this
             * runs in the timer task */
            synchronized( LicenseManagerImpl.this ) {
                if ( this.product.isActivated()) {
                    logger.info( "product is activated, expiring." );
                    this.product.expire();
                }
            }
        }
    ...
    static {
        ...
        /* This will actually run it twice, but the second call is
         * blocking until the licenses have been loaded */
