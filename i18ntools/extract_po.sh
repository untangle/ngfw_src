...
for module in untangle-node-directory-connector untangle-node-bandwidth untangle-node-configuration-backup untangle-node-branding untangle-node-spam-blocker untangle-node-virus-blocker untangle-node-wan-failover untangle-node-license untangle-node-policy untangle-node-web-filter untangle-node-wan-failover untangle-node-wan-balancer untangle-node-web-cache untangle-node-ipsec-vpn untangle-node-application-control
do
    module_dir=`echo "${module}"|cut -d"-" -f3`
