...
"untangle-apache2-config")
    ...
    xgettext --copyright-holder='Untangle, Inc.' -L Python -k_ -o tmp_keys.pot ../files/usr/lib/python2.6/*.py
    xgettext --copyright-holder='Untangle, Inc.' -L Python -k_ -o tmp_keys.pot ../files/usr/share/untangle/mod_python/error/*.py
    xgettext --copyright-holder='Untangle, Inc.' -L Python -k_ -o tmp_keys.pot ../files/usr/share/untangle/mod_python/auth/*.py
    msgmerge -U -N $1.pot tmp_keys.pot
    rm tmp_keys.pot
    update_po $1
    ;;
...
"untangle-node-openvpn")
    ...
    update_po $1
    ;;
"untangle-node-adconnector"|"untangle-node-bandwidth"|"untangle-node-boxbackup"|"untangle-node-branding"|"untangle-node-faild"|"untangle-node-policy"|"untangle-node-faild"|"untangle-node-splitd"|"untangle-node-webcache"|"untangle-node-ipsec"|"untangle-node-commtouchas"|"untangle-node-support"|"untangle-node-classd")
    moduleName=`echo "$1"|cut -d"-" -f3`
