...
char* jvector_error_throw( jvector_err_t type, const char* format, ... )
{
    ...
    /* Only throw an exception if there is not a pending exception */
     if ( jvector_exception() < 0 ) return (char*)format;
     
    switch ( type ) {
    case JVECTOR_ERROR_ARGS: exception = _jerror.illegal_arg; break;
    case JVECTOR_ERROR_STT:  exception = _jerror.illegal_stt; break;
    default:
    ...
    if ( exception == NULL ) {
        errlog ( ERR_CRITICAL, "jvector_error is unititalized\n" );
        return (char*)format;
    }
