...
public class SummaryGraph extends DayByMinuteTimeSeriesGraph
{
	...
	this("Traffic", true, true, "Clean/Passed", "Spyware Detected/Blocked", "Total Potential Spyware", "Web Requests/min.");
    }
	...
	sql = "SELECT date_trunc('minute', time_stamp) as time_stamp,"
	    + " count(*)" 
	    + " FROM tr_spyware_evt_cookie where"
	...
	// SETUP QUERY
	sql = "SELECT date_trunc('minute', time_stamp) as time_stamp,"
	    + " count(case blocked when true then 1 else null end),"
	    + " count(case blocked when false then 1 else null end)"
	    + " FROM tr_spyware_evt_access where"
	    + " time_stamp <= ? AND time_stamp >= ?"
	    + " GROUP BY time_stamp"
	    + " ORDER BY time_stamp";
	// RUN QUERY
	...
	while (rs.next()) {
	    // GET RESULTS
	    Timestamp eventDate = rs.getTimestamp(1);
	    long countA = rs.getLong(3);
	    long countB = rs.getLong(2);
	    long countC = rs.getLong(2) + rs.getLong(3);
	    // ALLOCATE COUNT TO EACH MINUTE WE WERE ALIVE EQUALLY
	    long eventStart = (eventDate.getTime() / MINUTE_INTERVAL) * MINUTE_INTERVAL;
	    long realStart = eventStart < startMinuteInMillis ? (long) 0 : eventStart - startMinuteInMillis;
	    int startInterval = (int)(realStart / MINUTE_INTERVAL)/MINUTES_PER_BUCKET;
	...
	sql = "SELECT date_trunc('minute', time_stamp) as time_stamp,"
	    + " count(*)" 
	    + " FROM tr_spyware_evt_activex where"
	    + " time_stamp <= ? AND time_stamp >= ?"
	    + " GROUP BY time_stamp"
	    + " ORDER BY time_stamp";
	// RUN QUERY
	...
	sql = "SELECT date_trunc('minute', time_stamp) as time_stamp,"
	    + " count(*)" 
	    + " FROM tr_spyware_evt_blacklist where"
	    + " time_stamp <= ? AND time_stamp >= ?"
	    + " GROUP BY time_stamp"
	    + " ORDER BY time_stamp";
	// RUN QUERY
