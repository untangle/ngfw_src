...
public class FTPConfigJPanel extends MEditTableJPanel {
    ...
    public FTPConfigJPanel(TransformContext transformContext) {
        super(true, true);
        ...
        FTPTableModel ftpTableModel = new FTPTableModel(transformContext);
...
class FTPTableModel extends MSortedTableModel{ 
    ...
    FTPTableModel(TransformContext transformContext){
    ...
    public Object generateSettings(Vector dataVector){
		...
		virusSettings.setFtpInbound(newElem);
	    }
	    else if( ((String)rowVector.elementAt(3)).equals("outbound") ){
		virusSettings.setFtpOutbound(newElem);
	    }
	    else{
