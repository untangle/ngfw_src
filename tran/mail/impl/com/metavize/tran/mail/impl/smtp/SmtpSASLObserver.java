...
import com.metavize.tran.mail.papi.smtp.NotAnSMTPResponseLineException;


/**
 * Class which acts to watch a SASL interaction,
 * attempting to detect if the SASL exchange will
 * result in a session with a negotiated security
 * layer (and thus we should enter passthru).
 * <br><br>
 * This class <b>will</b> eat the final server response
 * as-well, to prevent any synchronzation issues
 * with classes aligning requests/responses.
 */
class SmtpSASLObserver {

  /**
   * Enum of outcomes for every time
   * bytes are passed to instances
   * of this class
   */
  enum SmtpSASLStatus {
    /**
     * Exchange still in progress (i.e.
     * we do not know if it will be encrypted
     * or not).
     */
    IN_PROGRESS,
    /**
     * The Observer (either through explicit SASL
     * parsing of security layer negotiation or
     * parsing confusion) recommends that this
     * session enter passthru mode.
     */
    RECOMMEND_PASSTHRU,
    /**
     * The exchange is complete, and did not
     * result in the negotiation of a security layer.
     */
    EXCHANGE_COMPLETE
  };
  ...
  private final Logger m_logger =
    Logger.getLogger(SmtpSASLObserver.class);
  ...
  /**
   * Construct a SmtpSASLObserver based on
   * the SASLObserver.
   *
   * @param observer the SASLObserver
   */
  ...
  /**
   * The initial client response (junk on the
   * "AUTH XXXX..." line.  May be an "="
   * which means "no initial response".
   *
   * @param respStr the response String (may be null).
   *
   * @return the status
   */
  SmtpSASLStatus initialClientResponse(String respStr) {
    ...
    if(m_observer.initialClientData(ByteBuffer.wrap(bytes))) {
      return isKnownSecured()?
        SmtpSASLStatus.RECOMMEND_PASSTHRU:
        SmtpSASLStatus.IN_PROGRESS;
    }
    return SmtpSASLStatus.IN_PROGRESS;
  }

  /**
   * After this method completes, the bytes consumed
   * (i.e. the difference between the before/after positions
   * of the buffer) should be either (a) passed along
   * to the other side (an unparser), or (b) passed
   * along as some opaque token (a parser).
   * <br><br>
   * Bytes may remain in the buffer if the return is
   * either RECOMMEND_PASSTHRU or EXCHANGE_COMPLETE.
   * In the case of RECOMMEND_PASSTHRU simply pass
   * a copy of the original on.  In the case of
   * EXCHANGE_COMPLETE, pass along the consumed
   * bytes as opaque, then continue to parse
   * the buffer as SMTP data.
   *
   * @param buf the buffer of server data
   *
   * @return the status
   */
  SmtpSASLStatus serverData(ByteBuffer buf) {
    ...
    while(buf.hasRemaining()) {
      ...
      if((m_fromServerBBB.size() + dup.remaining()) > m_maxEncodedLineSz) {
        m_logger.warn("SASL response message exceeded max size of " +
          m_maxEncodedLineSz);
        return SmtpSASLStatus.RECOMMEND_PASSTHRU;
      }
      ...
      if(foundEOL) {
        ...
        try {
          ...
          if(resp.getCode() < 300) {
            //We're done.
            m_logger.debug("Ending SASL exchange with code \"" +
              resp.getCode() + "\" with session known to be clear: " +
                isKnownClear());
            return isKnownClear()?
              SmtpSASLStatus.EXCHANGE_COMPLETE:
              SmtpSASLStatus.RECOMMEND_PASSTHRU;
          }
          else if(resp.getCode() >= 400) {
            //We're done
            m_logger.debug("Ending SASL exchange with error code \"" +
              resp.getCode() + "\". Assume session not encrypted");
            return SmtpSASLStatus.EXCHANGE_COMPLETE;
          }
          else {
            m_logger.debug("Intermediate SMTP/SASL response code \"" +
              resp.getCode() + "\"");
          ...
          m_logger.warn("Unable to parse SMTP/SASL response line \"" +
            bbToString(respBuffer) + "\"", ex);
          return SmtpSASLStatus.RECOMMEND_PASSTHRU;
        }
      }
      else {
   ...
   * the buffer as SMTP data.
   *
   * @param buf the buffer of client data
   *
   * @return the status
   */  
  SmtpSASLStatus clientData(ByteBuffer buf) {
    ...
    while(buf.hasRemaining()) {
      ...
      if(append(dup, m_fromClientSB)) {
        m_logger.warn("SASL message exceeded max size of " +
          m_maxEncodedLineSz);
        return SmtpSASLStatus.RECOMMEND_PASSTHRU;
      }
      ...
      if(foundEOL) {
        ...
        if(bytes == null) {
          m_logger.warn("Null decoded client bytes.  Wait for server's response " +
            "in case this is a sneaky client attempting to get us to bypass " +
            "filtering");
        }
        else {
          if(m_observer.clientData(ByteBuffer.wrap(bytes))) {
            if(isKnownSecured()) {
              m_logger.debug("Base on reciept of client message and recomendation " +
                "of \"" + m_observer.getMechanismName() + "\" SASL Observer, enter passthru");
              return SmtpSASLStatus.RECOMMEND_PASSTHRU;
            }
          }
        }
  ...
  private byte[] base64Decode(String s) {
    ...
    try {
      ...
      m_logger.warn("Exception base 64 decoding \"" + s + "\" for SASL mechanism \"" +
        m_observer.getMechanismName() + "\" over SMTP", ex);
      return null;
    }
  }  
