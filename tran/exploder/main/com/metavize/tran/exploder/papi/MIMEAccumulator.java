 ...
 * and subtle, for a few reasons:
 * <ul>
 *   <li>
 *     "Ownership" of the underlying file is subtle.  Before a
 *     complete MIME message has been parsed, the accumulator
 *     owns the file.  Afterwards, users should consider the
 *     file "owned" by the MIME.  Once ownership of the file
 *     has been transfered, using the {@link #dispose dispose}
 *     method could hose callers.  Instead, use {@link #closeInput closeInput}
 ...
 *     are members of a given MIMEAccumulator, and have methods
 *     to determine if they should be unparsed.  Appending a chunk
 *     to the file is simplified via the {@link #appendChunkToFile appendChunkToFile}
 *     which silently ignores duplicate calls to append the same chunk.
 *   </li>
 *   <li>
 *     Exceptions are never thrown.  I find they confuse
 *     the calling code with a bunch of wrapped calls.  Instead,
 *     nulls (or booleans) convey success/failure.  In all cases
 *     where an error has occured, detailed logging is performed
 *     by instances of MIMEAccumulator.
 *   </li>
 * </ul>
 * <br><br>
 * If a MIMEAccumulator ever reaches an unparser, the
 * {@link #toTCPStreamer toTCPStreamer} method can be used to obtain
 * a streamer destined for the endpoint.  The MIMEAccumulator then
 * "remembers" this event, such that subsequent MIMEChunks may or may
 * not be written-out depending on when (if) they were appended to the
 * file.  If they were appended <i>before</i> the unparse event, then
 * they should not be unparsed if received.  In other words, please
 * use the "shouldUnparse" method on MIMEChunk before unparsing.
 * <br><br>
 * No one should ever see a MIMEChunk before the MIMEAccumulator to-which
 * it belongs.
 */
public class MIMEAccumulator {
  ...
  /**
   * Class used to represent a chunk of
   * MIME accociated with a given
   * MIMEAccumulator.  MIMEChunks
   * may not have useful data (if for example
   * they are simply a marker of
   * {@link #isLast the end} of a MIME
   * message).
   * <br><br>
   * MIMEChunks also may or may not
   * be written to file.  To write a given
   * chunk to a file, use the
   * "appendChunkToFile" method
   * of the MIMEAccumulator.
   * <br><br>
   * Before ever unparsing a MIMEChunk,
   * please use the {@link #shouldUnparse shouldUnparse}
  ...
  public class MIMEChunk {
    ...
    private MIMEChunk(ByteBuffer buf,
      boolean isLast,
      int index) {
      m_isLast = isLast;
      m_buf = buf;
    ...
    /**
     * Does this chunk represent the end
     * of the message.  Such chunks may
     * or may not {@link #hasData have data}.
     */
    ...
    /**
     * Does this chunk have any data.  Boundary
     * cases in parsing mayt result in blank
     * chunks.
     */
    ...
    /**
     * Get the underlying data.  Note that
     * you should first check {@link #hasData hasData}
     * to see if null may be returned.
     * <br><br>
     * <b>If unparsing, please first consult
     * {@link #shouldUnparse shouldUnparse}</b>
     */
    ...
    /**
     * Test if this chunk should be unparsed.  False will
     * be returned if this was passed t0
     * "appendChunkToFile" yet the MIMEAccumulator
     * has yet to be unparsed.
     */
    ...
    public void superDebugMe(Logger logger,
      String prefix) {
      String body = getData() == null?"<null>":com.metavize.tran.util.ASCIIUtil.bbToString(getData());
   ...
   * <br><br>
   * If exception thrown, no open files or descriptors
   * are left.
   *
  ...
  /**
   * Method to associate a chunk with this MIMEAccumulator.  Does
   * not implicitly {@link #appendChunkToFile add to the underlying file}.
   *
  ...
  /**
   * Add the given chunk to this accumulator.  An error
   * will occur if the chunk was not {@link createChunk created by this accumulator}.
   * <br><br>
  ...
  /**
   * Add bytes to the underlying file for the header.  Boolean
   * return conveys success.
   * <br><br>
   * Return of false means error, but things are
   * not closed.  Bytes in the buffer <b>are</b>
   * consumed.
   *
   * If <code>isLast</code> then the {@link #getHeadersLength headers length}
   * is assigned as the length of the file <b>after</b> this insert.  Passing
   * a null (or blank) buf with <code>isLast</code> true is the way
   * to assign null headers.
   *
   * Note that if there are any headers (or the headers were simply a
   * blank line) the terminating CRLF should be appended.
   *
   * @param buf the buffer (may be null)
   * @param isLast is this the last buffer for the headers
   *
   ...
   * <br><br>
   * This method may be called more than once, although after
   * the first call the headers are cached.
   * <br><br>
  ...
  public MIMEMessageHeaders parseHeaders() {
    ...
    if(getHeadersLength() == 0) {
      m_logger.debug("Parsing headers, yet no header bytes.  Assume " +
        "blank headers");
      
      m_headers = new MIMEMessageHeaders();
      return m_headers;
    }
  ...
  /**
   * Method called when we are {@link #parseHeaders parsing headers},
   * and encounter an exception parsing the headers.  Returns
   * any accumulated header bytes.  If there are none,
   * a blank buffer is returned.
   * <br><br>
   ...
   * <br><br>
   * return of null means you're hosed.  {@link #dispose Nuke}
   * the accumulator.  There is nothing else to do.
   *
  ...
  /**
   * Parse the accumulated bytes into a MIME
   * message.  This method may be called more
   * than once, yet the parsed message is cached.
   * <br><br>
   * Note that once parsed, the caller should assume
   * the MIMEMessage is responsible for the lifecycle
   * of the underlying file.
   * <br><br>
   * This method implicitly calls {@link #closeInput closeInput}.
   * <br><br>
   * Null is returned if there is an error, but the caller
  ...
  public MIMEMessage parseBody() {
    ...
    try {
      ...
      mimeIn.skip(m_headersLen);
      m_mimeMessage = new MIMEMessage(mimeIn,
        m_fileMIMESource,
        new MIMEPolicy(),
        null,
        parseHeaders());
   ...
   * in that it remembers that the Accumulator has
   * now been unparsed.
   *
  ...
  /**
   * Shuts down the accumulator for input.  Should
   * be called when the last chunk has been
  ...
  private boolean shouldUnparseImpl(MIMEChunk chunk) {
    if(m_unparsed) {
      ...
      return chunk.isWrittenToFile()?
        chunk.getIndex() > m_greatestChunkAppendedAndUnparsed:
        true;
    }
    else {
      return !chunk.isWrittenToFile();
