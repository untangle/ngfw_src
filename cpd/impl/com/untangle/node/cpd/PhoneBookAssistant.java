...
public class PhoneBookAssistant implements Assistant
{
	...
	/* ---------------- Package ----------------- */
	String addOrUpdate(InetAddress inetAddress, String username, Date expirationDate)
	{
		logger.debug( "addOrUpdate(" + inetAddress.toString() + ","
				+ username + "," + expirationDate + ")" );
		try {
			Username u = Username.parse( username );
			
			Data previous = userMap.put( inetAddress, new Data( inetAddress, u, expirationDate ));

			/* Expire the current lookup if it is not the same */
			UserInfo info = LocalUvmContextFactory.context().localPhoneBook().lookup( inetAddress );

			/* If the entry */
			if ( info != null && !u.equals( info.getUsername())) info.setExpirationDate( 0 );
	...
	String removeEntry( InetAddress address )
	{
	...
	private static class Data
	{
		...
		public String toString() {
			return address.getHostAddress() + "/" + username + "/" + this.expirationDate;
		}
	}
}
