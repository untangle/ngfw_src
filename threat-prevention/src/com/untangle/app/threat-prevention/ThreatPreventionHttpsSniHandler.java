...
public class ThreatPreventionHttpsSniHandler extends AbstractEventHandler
{
	...
	// if the license is not valid or
        ...
        SslEngineBase engine = (SslEngineBase) session.globalAttachment(AppSession.KEY_THREAT_PREVENTION_SSL_ENGINE);
        ...
        if (engine != null) {
            ...
            engine.handleClientData(session, data, AppSession.KEY_THREAT_PREVENTION_SSL_ENGINE);
            return;
        } else {
    ...
    private void checkClientRequest(AppTCPSession sess, ByteBuffer data)
    {
        ...
        if (redirect != null) {
            ...
            if(app.getSettings().getCloseHttpsBlockEnabled()){
                ...
                engine.handleClientData(sess, buff, AppSession.KEY_THREAT_PREVENTION_SSL_ENGINE);
            }
            return;
        }
