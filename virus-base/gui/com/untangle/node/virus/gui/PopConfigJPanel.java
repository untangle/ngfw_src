...
class PopTableModel extends MSortedTableModel<Object>{
    ...
    public TableColumnModel getTableColumnModel(){
        ...
        addTableColumn( tableColumnModel,  5, 10,    false, false, true,  false, VirusPOPConfig.class,  null, "");
        return tableColumnModel;
    }

    public void generateSettings(Object settings, Vector<Vector> tableVector, boolean validateOnly) throws Exception {
        VirusPOPConfig virusPopConfig = null;

        for( Vector rowVector : tableVector ){
            virusPopConfig = (VirusPOPConfig) rowVector.elementAt(5);
            virusPopConfig.setScan( (Boolean) rowVector.elementAt(2) );
            virusPopConfig.setMsgAction( (VirusMessageAction) ((ComboBoxModel)rowVector.elementAt(3)).getSelectedItem() );
            virusPopConfig.setNotes( (String) rowVector.elementAt(4) );
        }

        // SAVE SETTINGS ////////
        if( !validateOnly ){
            VirusSettings virusSettings = (VirusSettings) settings;
            virusSettings.setPopConfig( virusPopConfig );
        }
    }
    ...
    public Vector<Vector> generateRows(Object settings) {
        ...
        VirusPOPConfig virusPOPConfig = virusSettings.getPopConfig();
        row.add( super.ROW_SAVED );
        row.add( rowIndex );
        row.add( virusPOPConfig.getScan() );
        row.add( super.generateComboBoxModel(VirusMessageAction.getValues(), virusPOPConfig.getMsgAction()) );
        row.add( virusPOPConfig.getNotes() );
       row.add( virusPOPConfig );
        allRows.add(row);
