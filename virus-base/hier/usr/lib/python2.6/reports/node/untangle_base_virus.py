...
""" % (2 * (self.__vendor_name,))))
        ...
        ft = reports.engine.get_fact_table('reports.virus_ftp_totals')

        if not ft:
            ft = FactTable('reports.virus_ftp_totals', 'reports.ftp_events',
                           'time_stamp', [Column('hostname', 'text'),
                                          Column('username', 'text')], [])
            reports.engine.register_fact_table(ft)
...
""" % (self.__vendor_name,)
        ...
        try:
            ...
            if host:
                curs.execute(query_ftp, (one_week, ed, host))
            elif user:
                curs.execute(query_ftp, (one_week, ed, user))
            else:
                curs.execute(query_ftp, (one_week, ed))

            h3 = sql_helper.get_result_dictionary(curs)
...
class DailyVirusesBlocked(Graph):
    ...
    def get_graph(self, end_date, report_days, host=None, user=None, email=None):
        ...
        try:
            ...
            q, h = sql_helper.get_averaged_query(sums, "reports.ftp_totals",
                                                 start_date,
                                                 end_date,
                                                 extra_where = extra_where,
                                                 time_interval = time_interval)
...
class TopFtpVirusesDetected(Graph):
    def __init__(self, vendor_name):
        Graph.__init__(self, 'top-ftp-viruses-detected', _('Top Ftp Viruses Detected'))
        self.__vendor_name = vendor_name
...
ORDER BY sum DESC""" % self.__vendor_name
        ...
        try:
            curs = conn.cursor()
            if host:
                curs.execute(avg_max_query, (one_week, ed, one_week, ed, host, one_week, ed, host))
            elif user:
                curs.execute(avg_max_query, (one_week, ed, one_week, ed, user, one_week, ed, user))
            else:
                curs.execute(avg_max_query, (one_week, ed, one_week, ed, one_week, ed))
...
class VirusFtpDetail(DetailSection):
    def __init__(self, vendor_name):
        DetailSection.__init__(self, 'ftp-events',
                                       _('Ftp Events'))
        self.__vendor_name = vendor_name
