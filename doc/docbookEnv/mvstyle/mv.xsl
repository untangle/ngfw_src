...
<!-- 
      July 2004 Adapted from e-novative xsl by afolmsbee
-->
...
<!-- <xsl:param name="title.margin.left">.75in</xsl:param>
-->
...
<div id="customheader">
This is a custom header (<a href="http://yahoo.com" style="color: #fff; font-weight: bold;">Yahoo!</a>)
</div>
</xsl:template>
-->
...
<xsl:variable name="headerval">
 <xsl:choose>
  ...
  <xsl:when test="/article/articleinfo/title">
    <xsl:value-of select="/article/articleinfo/title"/>
  </xsl:when>
  <xsl:otherwise>
     <xsl:text>Metavize</xsl:text>
  </xsl:otherwise>
 </xsl:choose>
</xsl:variable>
