...
public class BlockJPanel extends MEditTableJPanel{
    ...
    class BlockTableModel extends MSortedTableModel<Object>{
        ...
        private static final int  C6_MW = 100; /* client interface */
        private static final int  C7_MW = 100; /* server interface */
        private static final int  C8_MW = 120;  /* source address */
        private static final int  C9_MW = 120;  /* destination address */
        private static final int  C10_MW = 110;  /* source port */
        private static final int  C11_MW = 110;  /* destination port */
        ...
        public TableColumnModel getTableColumnModel(){
            ...
            addTableColumn( tableColumnModel, 11, C11_MW, true,  true,  false, false, String.class, "2-5", sc.html("destination<br>port") );
            ...
            addTableColumn( tableColumnModel, 14, 10,     false, false, true,  false, FirewallRule.class, null, "");
            return tableColumnModel;
        }


        public void generateSettings(Object settings,
                                     Vector<Vector> tableVector,
            ...
            throws Exception
        {
            ...
            for( Vector rowVector : tableVector ){
                rowIndex++;
                newElem = (FirewallRule) rowVector.elementAt(14);
                ...
                newElem.setSrcIntf( (IntfDBMatcher)((ComboBoxModel)rowVector.elementAt(6)).getSelectedItem() );
                newElem.setDstIntf( (IntfDBMatcher)((ComboBoxModel)rowVector.elementAt(7)).getSelectedItem() );

                try{ newElem.setSrcAddress( IPMatcherFactory.parse((String) rowVector.elementAt(8)) ); }
                catch(Exception e){ throw new Exception("Invalid \"source address\" in row: " + rowIndex); }
                try{ newElem.setDstAddress( IPMatcherFactory.parse((String) rowVector.elementAt(9)) ); }
                catch(Exception e){ throw new Exception("Invalid \"destination address\" in row: " + rowIndex); }
                try{ newElem.setSrcPort( PortMatcherFactory.parse((String) rowVector.elementAt(10)) ); }
                catch(Exception e){ throw new Exception("Invalid \"source port\" in row: " + rowIndex); }
                try{ newElem.setDstPort( PortMatcherFactory.parse((String) rowVector.elementAt(11)) ); }
