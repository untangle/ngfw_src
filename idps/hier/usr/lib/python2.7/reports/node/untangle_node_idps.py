...
SELECT COALESCE(SUM(new_sessions),0)::int AS sessions,
       COALESCE(sum(CASE WHEN NULLIF(idps_description,'') IS NULL THEN 0 ELSE 1 END), 0) AS attacks,
       COALESCE(sum(idps_blocks), 0) AS blocks
...
class TopTenAttacksByHits(Graph):
    ...
    def get_graph(self, end_date, report_days, host=None, user=None,
        ...
        query = """\
SELECT idps_description, count(*) as hits_sum
FROM reports.session_totals
WHERE time_stamp >= %s::timestamp without time zone AND time_stamp < %s::timestamp without time zone
AND idps_description != ''"""
        ...
        query += " GROUP BY idps_description ORDER BY hits_sum DESC"
...
class IdpsDetail(DetailSection):
    ...
    def get_columns(self, host=None, user=None, email=None):
        ...
        rv = rv + [ColumnDesc('idps_ruleid', _('Rule ID')),
                   ColumnDesc('idps_description', _('Description')),
                   ColumnDesc('idps_blocked', _('Blocked')),
...
WHERE time_stamp >= %s::timestamp without time zone AND time_stamp < %s::timestamp without time zone
AND NOT idps_description ISNULL
AND idps_description != '' """ % (DateFromMx(start_date),
                                 DateFromMx(end_date)))
