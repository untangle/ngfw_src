def setup_detail()
  sql_helper().create_partitioned_table(<<DDL, 'time_stamp', @end_time) do |st, et|
...
INSERT INTO reports.n_http_events
    ...
    WHERE pe.time_stamp >= ? AND pe.time_stamp < ?
SQL
      ps.execute(st, et)
    end
  end

  ft = FactTable.new('reports.n_http_totals', 'reports.n_http_events',
                     'time_stamp',
  ...
  @report_engine.register_fact_table(ft)
end
