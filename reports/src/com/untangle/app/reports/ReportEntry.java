...
public class ReportEntry implements Serializable, JSONString
{
    ...
    public PreparedStatement toSql( Connection conn, Date startDate, Date endDate, String[] extraSelects, SqlCondition[] extraConditions, SqlFrom from, Integer limit )
    {
        ...
        from.setTable(LogEvent.schemaPrefix() + this.table);

        switch ( this.type ) {

        case PIE_GRAPH:
            return toSqlPieGraph( conn, startDate, endDate, from, extraSelects, allConditions );

        case TIME_GRAPH:
            return toSqlTimeGraph( conn, startDate, endDate, from, extraSelects, allConditions );

        case TIME_GRAPH_DYNAMIC:
            return toSqlTimeGraphDynamic( conn, startDate, endDate, from, extraSelects, allConditions );

        case TEXT:
            return toSqlText( conn, startDate, endDate, from, extraSelects, allConditions );

        case EVENT_LIST:
            return toSqlEventList( conn, startDate, endDate, from, allConditions, limit );
        }
    ...
    private PreparedStatement toSqlPieGraph( Connection conn, Date startDate, Date endDate, SqlFrom from, String[] extraSelects, LinkedList<SqlCondition> allConditions )
    {
        ...
        String pieQuery = "SELECT " +
            ...
            " FROM " +
            from.getFrom(getPieGroupColumn(), dateCondition + conditionsToString( allConditions ));
