 ...
 */
class FixedReportsQueue implements Runnable
{
    ...
    public synchronized void start()
    {
        ...
        logger.debug( "Starting Fixed Report queue" );

        /* If thread is not-null, there is a running thread that thinks it is alive */
        if ( thread != null ) {
            logger.debug( "Fixed Report queue is already running" );
            return;
        }
    ...
    {
        if ( thread != null ) {
            logger.debug( "Stopping Fixed Reports queue" );
