...
class RouterEventHandler extends AbstractEventHandler
{
    ...
    /**
     * Determine if a session is redirected, and if necessary, rewrite
     * its session information.
     */
    private boolean handleRedirect(IPNewSessionRequest request,
                                   Protocol protocol)
        throws MPipeException
    {
        /*
         * Check if this is a session redirect (A redirect related to
         * a session)
         */
        ...
        for (Iterator<RedirectMatcher> iter = redirectList.iterator();
             iter.hasNext();) {
...
class RequestIntfMatcher
{
    ...
    boolean isMatch(IPNewSessionRequest request)
    {
        ...
        return ((this.client.isMatch(request.clientIntf(),
                                     request.serverIntf(), c)
                 && this.server.isMatch(request.serverIntf(),
                                        request.clientIntf(), c)) ||
                (this.isBidirectional &&
                 this.client.isMatch(request.serverIntf(),
                                     request.clientIntf(), c)
                 && this.server.isMatch(request.clientIntf(),
                                        request.serverIntf(), c)));
    }
...
class NatMatcher
{
    ...
    boolean isMatch(IPNewSessionRequest request, Protocol protocol)
    {
        ...
        /* If the matcher matchers, or this fits the profile of a VPN
         * session, NAT it */
        return this.matcher.isMatch(request, protocol, c) ||
...
class DmzMatcher
{
    ...
    boolean isMatch(IPNewSessionRequest request, Protocol protocol)
    {
        ...
        return matcher.isMatch(request, protocol, c);
    }
