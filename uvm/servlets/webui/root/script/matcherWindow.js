...
//matcher pop-up editor for time groups
Ext.define('Ung.SqlConditionWindow', {
    extend:'Ung.MatcherEditorWindow',
    height: 150,
    width: 500,
    initComponent: function() {
        this.inputLines = [{
            ...
            items: [{
                ...
                editable: false,
                allowBlank: false,
                store: [["=","="], ["!=","!="], ["<>","<>"],["<","<"], , [">",">"], ["<=","<="], [">=",">="], ["between","between"], ["between","between"], ["like","like"], ["in","in"]]
            },{
                xtype: 'textfield',
                name: "value"
            }]
    ...
    setValue: function(value) {
        ...
        this.down('textfield[name="column"]').setValue(column);
        this.down('combo[name="operator"]').setValue(operator);
        this.down('textfield[name="value"]').setValue(val);
    },
    getValue: function() {
        var jsonobj = {
            column: this.down('textfield[name="column"]').getValue(),
            operator: this.down('combo[name="operator"]').getValue(),
            value: this.down('textfield[name="value"]').getValue(),
            javaClass: "com.untangle.uvm.SqlCondition"
        };
        return jsonobj;
    }
});