Ext.define('Ung.view.config.network.NetworkController', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.network',
    ...
    loadInterfaceStatusAndDevices: function () {
        ...
        rpc.networkManager.getInterfaceStatus(function (result, ex) {
            for (i = 0; i < interfaces.length; i += 1) {
                ...
                intfStatus = Ext.Array.findBy(result.list, function (item) {
                    return item.interfaceId === vm.get('settings.interfaces.list')[i].interfaceId;
                });
        ...
        rpc.networkManager.getDeviceStatus(function (result, ex) {
            for (i = 0; i < interfaces.length; i += 1) {
                ...
                devStatus = Ext.Array.findBy(result.list, function (item) {
                    return item.deviceName === interfaces[i].physicalDev;
                });
                if (devStatus) {
                    Ext.apply(interfaces[i], {
