...
def log_date( cmd ):
    p = subprocess.Popen(["date","+%Y-%m-%d %H:%M"], stdout=subprocess.PIPE, universal_newlines="text")
...
def cmd_to_log(cmd):
    ...
    p = subprocess.Popen(["sh","-c","%s 2>&1" % (cmd)], stdout=subprocess.PIPE, stdin=stdin, universal_newlines="text")
    for line in iter(p.stdout.readline, ''):
        log( line.strip() )
    p.wait()
    return p.returncode
...
def update():
    ...
    p = subprocess.Popen(["sh","-c","apt-get update %s 2>&1" % UPDATE_OPTS], stdout=subprocess.PIPE, universal_newlines="text")
    for line in iter(p.stdout.readline, ''):
...
def check_upgrade():
    p = subprocess.Popen(["sh","-c","apt-get -s dist-upgrade %s 2>&1" % UPGRADE_OPTS], stdout=subprocess.PIPE, universal_newlines="text")
    for line in iter(p.stdout.readline, ''):
