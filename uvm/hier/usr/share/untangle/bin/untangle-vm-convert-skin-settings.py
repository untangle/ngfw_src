...
def get_settings(debug=False):
    if (debug):
        print "Getting u_skin_settings"

    node_settings = sql_helper.run_sql("select admin_skin from u_skin_settings ", debug=debug)

    if node_settings == None:
        print "WARNING: missing u_skin_settings result"

    if len(node_settings) > 1:
        print "WARNING: too many u_skin_settings results (%i)" % (len(node_settings))

    skin = node_settings[0][0]
